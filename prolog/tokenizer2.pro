...
tokenize( ?str, ?tokens ) :- tokenize( ?str, ?tokens, _).
...
tokens( [?inherit | ?inherits] ) -->
   { ?inherit == reserved(inherit) },
   !, inherit_tokens( [?inherit | ?inherits] ).
tokens( [wh(?w) | ?tokens] ) -->
   { nonvar(?w) },
   wh( wh(?w) ),
   !, tokens(?tokens).
...
inherit_tokens( [?tok_inherit, wh(?w), ?tok_from | ?toks_modules] ) -->
   token(?tok_inherit),
   wh( wh(?w) ),
   token(?tok_from),
   inherit_module_tokens(?toks_modules).

inherit_module_tokens([wh(?w), text(?mod) | ?toks_modules]) -->
   wh( wh(?w) ),
   token(?mod),
   !, inherit_module_tokens( ?toks_modules ).
inherit_module_tokens([]) --> [].
...
%   wh,
   anAtom( ?token ),
   !.
% when going backward, push reserved back as atoms
token( reserved(?word) ) -->
   { nonvar(?word) },
   { lookup(?a, ?word) },
%   wh,
...
next_non_number -->
   ...
   !, fail.
%   token_error(`Invalid number`).   % can't throw, slows down get_vba_value reading array entries
next_non_number --> [].
...
%--------------------------------------------
% Atoms - no single quotes - just pure atoms
%
...
%--------------------------------------------
% Text Values anything quoted
%

toText(?S, ?Cs) :- length(?Cs, ?l), ?l < 60, atom_codes(?S, ?Cs), !.
toText(?S, ?Cs) :- string_list(?S, ?Cs).

aString(text(?S)) --> { var(?S) }, quoted_chars(0'`, ?Cs), { toText(?S, ?Cs) }, !.   % `
aString(text(?S)) --> { var(?S) }, quoted_chars(0'", ?Cs), { toText(?S, ?Cs) }, !.   % "
aString(text(?S)) --> { var(?S) }, quoted_chars(0'', ?Cs), { toText(?S, ?Cs) }, !.   % '
aString(text(?S)) --> { string(?S) }, { string_list(?S, ?Cs) }, quoted_chars(0'", ?Cs).   % "
aString(text(?S)) --> { atom(?S) }, { atom_codes(?S, ?Cs) }, quoted_chars(0'", ?Cs).   % "
