...
type_conversions([], _, []) :- !.
type_conversions([?S| ?Ss], [], [?T| ?Ts]) :-
  type_convert(?S, ?T),
  !,
  type_conversions(?Ss, [], ?Ts).
...
type_convert(?S, ?N) :- string_number(?S, ?N), !.
type_convert(?S, ?A) :- string_length(?S, ?L), ?L > 50, string_atom(?S, ?A), !.
type_convert(?S, ?S).
